...
Live UI 是可选的本地观察界面;核心路由不需要 Python,只有这个界面需要
Python 3.9 或更高版本。它只绑定 `127.0.0.1`,使用随机令牌保护,并且全程只读。
界面会显示每个作业的任务(`task.txt`)和公开结果(`out.txt`),不会显示工作端或
厂商的原始日志。需要时才运行 `omnilane ui start`,使用完毕后请运行
`omnilane ui stop`。
