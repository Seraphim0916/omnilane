...
| arbitrate | vote: codex+claude+grok | 内置意见评审团,重大决定用——每评审每轮烧一次额度;主控模型当主席下裁决 |
...
设置菜单和 `routing.local.yaml` 就是让你不同意用的。想换成自己的多模型
审查闸,用 `exec` 厂商指向你的脚本;嫌评审团贵,在 `routing.local.yaml`
写 `arbitrate: off - -` 关掉。
