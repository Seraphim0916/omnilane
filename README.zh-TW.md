...
| arbitrate | vote: codex+claude+grok | 內建意見評審團,重大決定用——每評審每輪燒一次額度;主控模型當主席下裁決 |
...
**重大決定開評審團,不是問一個人。**`arbitrate` 通道預設就是內建的 `vote`
廠商:同一個問題丟給每個已安裝的評審(`arbitrate: vote codex,claude,grok -`),
意見並排回來,由發問的主控模型當主席下裁決。檔位欄填 `2` 開辯論輪——
每個評審看完整個評審團的意見,只針對分歧互駁。想開四人團就加 `gemini`。
代價是每評審每輪燒一次額度——嫌貴就在 `routing.local.yaml` 寫
`arbitrate: off - -` 關掉。進階使用者可用
