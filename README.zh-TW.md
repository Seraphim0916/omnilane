...
Live UI 是選配的本機觀察介面;核心路由不需要 Python,只有這個介面需要
Python 3.9 以上。它只綁 `127.0.0.1`,以隨機 token 保護,而且全程唯讀。
畫面會顯示每個工作的任務(`task.txt`)與公開結果(`out.txt`),不會顯示工作端或
廠商的原始 log。要用時才跑 `omnilane ui start`,用完請跑
`omnilane ui stop`。
