...
一張路由表,四個 harness 通用。omnilane 讓**任何**一個 agentic CLI——Claude Code、
OpenAI Codex、Grok Build、Google Antigravity——的主迴圈把子任務分類到 lane,
再自動把每條 lane 派工給該項工作最強的廠商 CLI,直接沿用你既有的訂閱登入。
...
- **`routing.yaml`** — lane → 廠商+模型+推理檔位。任何一條 lane 都可以在
  `~/.omnilane/routing.local.yaml` 覆寫(本機優先)。
- **`scripts/dispatch.sh <lane> "<任務>"`** — 查表後以無頭(headless)方式呼叫
  對應廠商的 CLI。長任務加 `--background`,之後用 `scripts/jobs.sh` 查狀態、取結果;
  工作端需要改檔案時用 `--mode work --workdir 目錄`。
- **`skills/omnilane/SKILL.md`** — 一份 skill 四個 harness 都能載入:
  先認出自己是哪個模型,自己 lane 的活自己做,其餘派出去。

## Lane 一覽(預設值;實際生效值看 routing.yaml)
...
最快:`./install.sh` — 自動偵測本機有哪些 CLI,幫你接好 skill 並列出其餘的
plugin 安裝指令(`--uninstall` 可逆)。手動接線:

- **Claude Code**:以 plugin 安裝(附 `/route`、`/route-jobs` 指令),
...
(每個 runner 都會載入;永不進版控)——參考 `local.sh.example` 與
...
- 同一個工作目錄的 codex 派工**自動序列化**(避免並行互撞)。
- 任務酬載長度上限,防止爆量。

## 預設值與資料來源

預設 lane 配置依據 Artificial Analysis 2026-07 的編碼/智力資料與公開的
對比評測;這些是意見不是定律——照你的偏好改 `routing.yaml`。
`arbitrate` lane 預設關閉:有自己的多模型審查閘就接上去。

## 狀態

早期版本。runner 介面已穩定;Grok/Antigravity 的指令殼行為可能隨 CLI
