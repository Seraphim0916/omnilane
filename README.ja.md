...
- **自然言語で質問** — `omnilane` スキルと `/route` は、モデルの推薦、
  ベンダー指定の相談、標準モデルファミリーの固定に対応し、暗黙の切り替えをしません。
- **作業をリアルタイム表示** — 認証付き・読み取り専用のローカルワークベンチで、
  デスクトップとモバイルからジョブ状態、タスク、ルーティング判断、公開結果を確認できます。
- **ハング防止時間を作業別に調整** — `--timeout` で単発、
  `OMNILANE_TIMEOUT_<LANE>` でレーン別、`OMNILANE_TIMEOUT` で全体を設定できます。
...
omnilane doctor                                # ルーティングとローカル実行環境を読み取り専用で診断
...
Live UI は任意のローカルワークベンチです。コアルーティングに Python は不要で、
この UI のみ Python 3.9 以降が必要です。
...
再接続します。`127.0.0.1` のみにバインドし、ランダムトークンで保護された
読み取り専用画面です。`task.txt` と公開用 `out.txt` のみを表示し、ワーカーや
ベンダーの生ログは表示しません。
...
- **ジョブ全体ヒューズ** — 任意の `--job-timeout SECONDS` はロック待ち、
  ...
  `OMNILANE_JOB_TIMEOUT` > 無効。期限切れは監視対象を終了して 124 を返します。
...
v0.4.0 は、レビュー済みのルーティングコア、自然言語コンサルテーション、
ローカル Live UI を統合します。shell コアは外部モデルレビュー(11 件の指摘を
全修正)と敵対的検証を通過し、UI は Python テストと実ブラウザの動作確認で
検証済みです。Grok/Antigravity のコマンドシェル挙動は CLI バージョンで変わる
可能性があります。issue と PR を歓迎します。
