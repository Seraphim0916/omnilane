...
Live UI は任意のローカル観察画面です。コアルーティングに Python は不要で、
この UI のみ Python 3.9 以降が必要です。`127.0.0.1` のみにバインドし、
ランダムトークンで保護された読み取り専用画面です。各ジョブのタスク
(`task.txt`)と公開結果(`out.txt`)を表示しますが、ワーカーやベンダーの生ログは
表示しません。必要なときだけ `omnilane ui start` で起動し、利用後は
`omnilane ui stop` で停止してください。
