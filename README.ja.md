...
サーバーは `route` に加えて、読み取り専用の照会ツール `list_lanes`、`explain`、`validate`、`dry_run`、`jobs_list`、`jobs_status`、`jobs_result`、`doctor` を公開します。
...
runner 契約と契約テスト付き。Claude Code の `SessionStart` 自動リマインダーと
MCP stdio サーバー(`omnilane mcp`)も同梱。kimi、qwen、opencode、openrouter の runner はフェイクバイナリで
