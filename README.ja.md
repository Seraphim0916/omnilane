...
`omnilane` スキルまたは `/route` では、普通に **「Opus にこのアーキテクチャを
厳しく検討してもらって。」** と頼めます。自然言語を解釈するのは Agent Skill
であり、`dispatch.sh` に自由文の shell パーサーを追加するものではありません。
...
または設定済みの指定ベンダー CLI が利用不可、`86` ネストディスパッチ拒否、
`87` ロック待ちタイムアウト。
