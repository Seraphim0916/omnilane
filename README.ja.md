...
- **ゴール台帳。** `omnilane goal open` は境界付きのゴール台帳を作成します。`goal dispatch` は各ジョブの実行前にジョブ数上限、経過時間の予算、繰り返し失敗を止めるヒューズを確認します。`goal note` は呼び出し元の記録を残し、`goal status` は予算とジョブごとの記録を表示し、`goal close` は `goals/<id>/report.md` を書き込みます。
