...
|---|---|---|---|---|
| hardest-coding | Coding capability | Sol xhigh → Opus xhigh | Fable 5.1 xhigh → Sol xhigh | Fable leads Sol on Terminal-Bench 91.0 vs 89.5 and SciCode 60.1 vs 56.0. Anthropic also puts Fable above Opus on Terminal-Bench 4.0 and CursorBench; Sol's coding component costs about one sixth as much. |
...
| hard-judgment | Agentic knowledge work | Opus xhigh → Sol max | Fable 5.1 xhigh → Sol max → Grok 4.6 | Fable leads Opus on Agentic 59.8/58.4, GDPval 1835/1797, and Omni 42.4/35.4 at about 47% more per task (2.651/1.801). Grok's Agentic 58.7 is near Sol's 57.8 and its .34 hallucination rate is much lower than Sol's .92, but its effective CLI effort is unknown. |
| taste-final | Prose and polish | Opus high → Sol max | Fable 5.1 high → Sol max | At the same effort Fable leads Intel 62.5/61.5 and Omni 40.8/33.7 for about 17% more per task (1.430/1.227). Opus retains the lower hallucination rate, .61 vs .69. |
...
| ui-draft | Coding and measured multimodality with references | Sol xhigh → Opus high | Sol xhigh → Fable 5.1 high | Fable high beats Opus high on Intel 62.5/61.5 and Coding 79.1/76.5. MMMU-Pro is unmeasured for Fable; measured rows are Opus high .82, Sol xhigh .83, and Flash .85. |
...
| fast-agentic | Interactive multi-step latency | Luna max → Gemini 3.6 Flash High | Gemini 3.7 Flash Medium → Luna high | Luna max has Agentic 46.9 vs Flash 45.1, but TTFA is 101 vs 5.4 seconds and throughput 128 vs 282 tok/s. Luna high is the Codex fallback on TTFA, 11 seconds vs xhigh's 41. |
| live-search | Native live X/web access | Grok 4.6 → off | Grok 4.6 → off | Unchanged: the lane is defined by Grok's native live-search surface rather than an AA score. |
