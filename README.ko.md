...
모든 서브태스크를 가장 잘하는 모델에게 자동 배분——<br/>
**Claude Code · Codex · Grok Build · Antigravity**, 기존 구독 그대로.
...
omnilane 은 **어떤** agentic CLI든——Claude Code, OpenAI Codex, Grok Build,
Google Antigravity——메인 루프가 서브태스크를 레인(lane)으로 분류하고,
각 레인을 그 작업에 가장 강한 벤더 CLI 로 자동 디스패치하게 해 줍니다.
기존 구독 로그인을 그대로 사용합니다.

```
              ┌────────────── routing.yaml(테이블 하나)───────────┐
 메인 루프  ─┤ hardest-coding → Codex Sol      taste-final → Claude │
 (아무 CLI)  │ bulk-mechanical → Codex Terra   long-context → Gemini│
...
## 레인 목록(기본값. 실효값은 `scripts/dispatch.sh --list`)

| 레인 | 1순위 모델 | 용도 |
|---|---|---|
| hardest-coding | GPT-5.6 Sol (xhigh) | 가장 어려운 구현, 근본 원인 디버깅, 정확성이 핵심인 수정 |
| bulk-mechanical | GPT-5.6 Terra (max) | 리팩터링, 마이그레이션, 테스트, 대량 스윕 |
| triage | GPT-5.6 Luna (medium) | 대량 1차 선별 |
| hard-judgment | GPT-5.6 Sol (max) | 아키텍처 중재, 깊은 추론, 세컨드 오피니언 |
| taste-final | Claude Opus 4.8 | 대외 문장, prompt/문서 다듬기, 스타일 최종심 |
...
| arbitrate | off(옵트인) | 내장 의견 패널(중대한 결정용)——기본 비활성. `routing.local.yaml` 에서 활성화;투표자×라운드마다 1콜 소모 |
